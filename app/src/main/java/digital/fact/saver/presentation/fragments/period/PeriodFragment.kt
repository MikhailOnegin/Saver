...
class PeriodFragment : Fragment() {
    ...
    private fun onSourcesChanged(sources: List<Source>?) {
        sources?.let {
            ...
            binding.balanceIncome.text = result.second.first.toStringFormatter()
            binding.balanceExpenses.text = result.second.second.toStringFormatter()
        }
        ...
        binding.periodDayBalance.text =
            ((if (daysCount == 0) activeSummary else activeSummary.div(daysCount)).toStringFormatter())
    }
